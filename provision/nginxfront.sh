...
rdr inet  proto tcp from any to <ext_ip4> port { 80 443 } -> $(get_jail_ip  nginxfront)
rdr inet6 proto tcp from any to <ext_ip6> port { 80 443 } -> $(get_jail_ip6 nginxfront)
EO_PF_RDR

	configure_pf_jail_table nginxfront

	store_config "$_pf_etc/filter.conf" <<EO_PF_FILTER
pass in quick proto tcp from any to <nginxfront> port { 80 443 }
EO_PF_FILTER
}
