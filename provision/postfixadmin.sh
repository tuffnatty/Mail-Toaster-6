...
postfixadmin_init_db()
{
	...
	start_postfixadmin
	fetch -o - "http://$(get_jail_ip stage)/setup.php"
	stage_exec postfixadmin-cli admin add "$TOASTER_ADMIN_EMAIL" --superadmin 1 --active 1 --password "$TOASTER_ADMIN_PASS" --password2 "$TOASTER_ADMIN_PASS" ||
		echo "$(red)WARNING: could not add admin$(normal)"
	stage_exec postfixadmin-cli domain add "$TOASTER_MAIL_DOMAIN" ||
		echo "$(red)WARNING: could not add domain$(normal)"
}

install_postfixadmin()
{
	install_php "$pa_php_ver" "imap" || exit 1
	...
	sed -i.bak \
		...
		-e 's,/%postfixadmin-cli/,%postfixadmin-cli,' \
		"$STAGE_MNT$postfixadmin_root/scripts/postfixadmin-cli"
	stage_exec ln -s $postfixadmin_root/scripts/postfixadmin-cli /usr/local/bin/postfixadmin-cli

	tell_status "installing dovecot (only for doveadm pw, should be no longer needed after PostfixAdmin 4)"
...
"
	...
	configure_nginx_server_d postfixadmin
}
...
configure_postfixadmin()
{
	...
	tell_status "configuring local dovecot so that doveadm pw is working"
	...
	sed \
		...
		-e "/^\\\$CONF\\['dovecotpw'/		s,/usr/local/sbin/doveadm pw,/usr/local/bin/doveadm pw," \
